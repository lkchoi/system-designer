...
const CONSTANTS: Constant[] = [
  ...
  { name: 'Network RTT (cross-region)', value: '~50\u2013100 ms', tags: 'network latency cross region' },
  { name: 'Network RTT (cross-continent)', value: '~100\u2013300 ms', tags: 'network latency continent global' },
...
const ALL_TECHNOLOGIES: FlatTech[] = (Object.keys(TECHNOLOGY_CATALOG) as ComponentType[]).flatMap(
  category => TECHNOLOGY_CATALOG[category].map((t: TechnologyInfo) => ({
    name: t.name,
    category,
...
export default function CapacityCalculator({ open, onClose }: Props) {
  ...
  const filteredTech = useMemo(() => {
    ...
    const q = search.toLowerCase();
    return ALL_TECHNOLOGIES.filter(t =>
  ...
  return (
    <div className="capacity-overlay" onClick={onClose}>
      <div className="capacity-card" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Escape') onClose(); }}>
            ...
            {results ? (
              <div className="capacity-results">
                ...
                <ResultGroup title={`With Replication (\u00d7${rf})`}>
                  ...
                  <ResultRow label="Per month" value={formatDataSize(results.monthlyReplicatedGB)} />
                ...
                <ResultGroup title="Bandwidth">
                  ...
                  <ResultRow label={`With replication (\u00d7${rf})`} value={formatBandwidth(results.bandwidthReplicatedMbps)} />
                </ResultGroup>
                ...
                <ResultGroup title="Storage Projections">
                  ...
                  <ResultRow label={`Retention (${ret}d)`} value={formatTB(results.retentionStorageTB)} />
                </ResultGroup>
